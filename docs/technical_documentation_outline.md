...
- 重复计时
  使用`--online-repeats N`可在一次setup后重复同一在线窗口，报告首轮、中位数、P90、末轮和预测一致率。首轮包含影子监测与可能的升档重算，末轮更接近稳定精度配置下的光计算模拟器耗时。
...
- 模拟器输出同步
  自适应精度与固定8-bit对照均在光计算模拟器上执行。由于部分开发环境无法直接调用模拟器，由实验端运行后将backend标识、预测、误差、tile/slice次数和耗时记录到`artifacts/`。数字调试回退数据不作为正式结果，当前模拟器数据待完善。

- 初步跨窗口验证
  当前待完善数据为：3个evaluation窗口中两者command accuracy均为1.000、reject rate均为0、预测一致率为1.000，平均概率L2差为0.00775；自适应平均tile下降15.7%，中位时延约1.540 s，对照约1.634 s。重复窗口数据为15.1%稳态tile降幅、1.545 s中位时延和1.600 s P90。上述数据和精度策略仍待完善；即使完成少量窗口验证，也只能作为工程sanity check，正式结论必须扩大到完整被试与session。
...
- 光计算硬件未实测  
  当前 photonic backend 对接光计算模拟器，模拟器结果不是真实光芯片功耗/延迟结果。
...
- 真实硬件执行尚未完成  
  FBCSP、滤波、标准化、MLP、线性头扫描和概率融合等前向线性路径已经统一暴露并交由光计算 backend 接管，当前证据来自光计算模拟器、接口路由与 MAC-equivalent 统计，尚未在真实光芯片上逐算子完成执行与测量。
...
- 真实光芯片接入  
  保持上层算法不变，实现真实硬件驱动并将当前光计算模拟器 backend 迁移到实机，逐算子验证全部前向线性计算的真实芯片执行、数值误差、延迟和功耗。
