...
CAR 重参考 & $X'=(I-\frac1C\mathbf{1}\mathbf{1}^\top)X$，$8\times8$ 乘 $8\times T$ & $4T$ & 可以切块；在线时先在数字端完成，方便处理滤波状态和坏通道 \\ \hline
OVR CSP 投影 & $Z=W X$，$4\times8$ 乘 $8\times T$ & $2T$/频带/类别 & 可以切块；6 个频带、3 类共 $36T$，调用次数较多 \\ \hline
特征标准化 & $[D\;c][x;1]$，$32\times33$ 乘 $33\times1$ & 80 & 可以切块；后续可和编码器一起加入 \\ \hline
MLP 编码器 & $W_1[\bar{x};1]$：$64\times33$；$W_2[h_1;1]$：$32\times65$ & 160 + 144 & 可以切块；GELU/dropout 在数字端计算 \\ \hline
候选线性头 & $[A_i\;b_i][h;1]$，$8$ 个 $3\times33$ 乘 $33\times1$ & \textbf{80} & 首选 LTSimulator 热点；规则、批量、直接影响候选选择 \\ \hline
候选融合 & $p=\alpha^\top P$，$1\times8$ 乘 $8\times3$ & 2 & 数字端执行，便于做概率校准和拒识 \\ \hline
\end{tabularx}
\end{table}
