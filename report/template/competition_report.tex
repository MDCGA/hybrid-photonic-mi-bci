...
\vspace{0.2cm}
\noindent\textbf{当前证据。} 在 BCICIV\_1\_asc 回放实验中，42 个校准窗口仅用于经验库查询和候选加权，其余 518 个窗口用于在线评估；当前主线的指令准确率为 $70.46\%$，平衡准确率为 $73.24\%$，接受窗口准确率为 $72.42\%$，拒识率为 $2.70\%$。在 BNCI2014\_004 的 9 位被试实验中，主线平均准确率为 $74.92\%$，拒识率为 $1.62\%$。代码、测试、指标、单窗口推理入口和图表均已保存；候选头路径已完成量化、$2\times8$ tile 调度、输出读回和计算量统计。Cyton 在线采集与上位机仍在完善，尚未完成真实 EEG 采集--推理--反馈闭环；当前结论均来自公开数据集回放和软件后端。
...
\begin{table}[H]
\centering\caption{项目完成情况与可展示材料}\label{tab:status}
...
$2\times8$ tile 扫描 & 已完成软件模拟 & 分块累加实现、tile 计数、与浮点参考的一致性测试 \\ \hline
...
当前通过 MatrixOps 路由的主要步骤包括：FBCSP/CSP 空间投影与协方差乘积、特征标准化仿射、MLP 前向线性层、LDA 参数与分类分数、经验库 centroid 距离交叉项以及候选概率线性融合。激活、方差、对数、softmax、阈值和矩阵分解不属于该接口。参考实现 \texttt{NumpyMatrixOpsBackend} 直接使用 NumPy；photonic 软件实现负责量化、$2\times8$ tile 或 4-bit slice 拆分、整数乘法、零点补偿和反量化。当前仍是软件执行核，尚未替换为真实光芯片驱动。
...
主线把 8 个 $3\times33$ 增广候选头交给该接口。默认 tile 为 $2\times8$，所以一次 scan 需要 $8\lceil3/2\rceil\lceil33/8\rceil=80$ 次 tile。每个 tile 内部调用量化 MatrixOps backend；候选路径默认单次 4-bit，偏置作为常数输入一并扫描。\texttt{last\_tile\_count} 和 \texttt{count\_tiles} 提供调度证据。接口既可回退到 NumPy 整数矩阵乘，也可调用已加载的 Gazelle 模拟器；真实芯片的传输、标定和读回仍需未来驱动实现。

三个接口共同建立“算法 shape 不受物理 tile 限制”的边界。当前候选扫描使用单次 4-bit，其他前向线性算子采用 8-bit 逻辑精度的 4-bit 位权拆分；必须继续区分 backend 路由、量化配置和真实芯片逐算子实测三个证据层次。
...
\section{逐算子矩阵乘拆分}
为了能复查光计算比例，我们不只看模块名称，而是逐个列出实际的线性计算。表~\ref{tab:matrix-split}给出主线单个窗口的拆分方式。``可映射'' 表示矩阵大小可以按 tile 切分；计入光计算时，需要有 LTSimulator 的调用、读回结果、量化配置和日志记录。
\begin{table}[H]\centering\small
\caption{主线算法到光子矩阵乘的拆分}\label{tab:matrix-split}
\begin{tabularx}{0.96\linewidth}{|L{0.18\linewidth}|L{0.50\linewidth}|Y|}\hline
算法步骤 & 矩阵表达式与大小 & $2\times8$ tile 数 \\ \hline
CAR 重参考 & $X'=(I-\frac1C\mathbf{1}\mathbf{1}^\top)X$，$8\times8$ 乘 $8\times T$ & $4T$ \\ \hline
OVR CSP 投影 & $Z=W X$，$4\times8$ 乘 $8\times T$ & $2T$/频带/类别 \\ \hline
特征标准化 & $[D\;c][x;1]$，$32\times33$ 乘 $33\times1$ & 80 \\ \hline
MLP 编码器 & $W_1[\bar{x};1]$：$64\times33$；$W_2[h_1;1]$：$32\times65$ & 160 + 144 \\ \hline
候选线性头 & $[A_i\;b_i][h;1]$，$8$ 个 $3\times33$ 乘 $33\times1$ & 80  \\ \hline
候选融合 & $p=\alpha^\top P$，$1\times8$ 乘 $8\times3$ & 2 \\ \hline
\end{tabularx}
\end{table}
...
\end{equation}
候选 head scan 直接采用一次 4-bit 量化调用。其他前向线性路径先量化为 8-bit 逻辑输入/权重，再拆成多个 uint4/int4 slice 重构；因此“8-bit 逻辑精度”不表示一次原生 uint8/int8 光计算调用。
...
CAR、SOS 状态转移 & 8-bit 逻辑值 & 多个 uint4/int4 slice \\ \hline
FBCSP、标准化、MLP、LDA & 8-bit 逻辑值 & 多个 uint4/int4 slice \\ \hline
距离交叉项、概率融合 & 8-bit 逻辑值 & 多个 uint4/int4 slice \\ \hline
...
\subsection{Radix-16 位权拆分与 8-bit 逻辑精度}
普通前向线性路径需要更大的逻辑动态范围。无符号输入整数按 radix-16 展开为
\begin{equation}
...
这种 positional decomposition 保留 8-bit 逻辑精度，区别于把 8-bit 值直接截断为 4-bit。代价是更多 slice 调用、部分和累加、潜在噪声传播和更高延迟；它是当前软件接口的默认折中，不代表所有算子都必须使用 8-bit。

\subsection{当前实现与证据边界}
其他 MatrixOps 路径由 \texttt{BitSlicedPhotonicMatrixOpsBackend} 接管，默认 logical input/weight bits 均为 8，physical slice bits 固定为 4。SignalOps 中 CAR 展开为通道混合矩阵；SOS 的二阶节用状态转移形式表达，其系数乘加调用同一 bit-sliced MatrixOps 路径。候选 scan 则由 \texttt{QuantizedPhotonicMatrixOpsBackend} 完成单次 4-bit tile 计算。
...
软件会优先尝试加载可选的 Gazelle 模拟器，未加载时回退到 NumPy 整数矩阵乘。本次单窗口终端结果已确认模拟器加载成功；现有证据包括 backend 标识、量化参数、tile/slice 调度和阶段耗时，不包括真实 ADC/DAC、器件标定、功耗或芯片时延。

\subsection{精度、抗噪与资源权衡评估}
后续不能假设所有算子需要相同逻辑位宽。应分别扫描 CAR、SOS、FBCSP 投影、标准化、MLP 各层、距离交叉项、候选头和概率融合，比较浮点参考与量化输出误差，并在相同数据划分、模型参数和拒识协议下记录 command accuracy、balanced accuracy、accepted accuracy 和 reject rate。

逐算子的目标是找到满足任务指标与数值稳定性的最低逻辑精度。对滤波状态转移、FBCSP 或敏感 MLP 层可保留较高位宽；对候选扫描或融合等容忍度较高的路径使用更低位宽，形成 precision policy。建议逻辑位宽比较 4/6/8/10/12-bit，物理位宽比较 4/5/6/8-bit，并加入不同噪声等级。
...
\end{equation}
估算得到单窗口板端计算约 0.42 s，相当于约 2.39 个窗口每秒，明显短于当前运动想象任务约 3 s 的 EEG 决策窗口。在上述算力解释和连续供给前提下，评估板具备单窗口实时推理的计算时间条件；476.405 s 的模拟器耗时反映模拟过程较慢，不代表实机运行速度。最终端到端时延仍需在评估板上计入通信、调度和数字端处理后实测。
...
当前 photonic backend 是软件量化、切片和接口路由，不是真实光芯片的逐算子执行证据。100\% forward linear share 表示全部前向线性 MAC 已按工程口径交给 backend 接管，不等于真实功耗占比、吞吐或端到端加速比。候选扫描使用单次 4-bit；其他路径采用 8-bit 逻辑精度的 4-bit 位权拆分，尚需系统评估 slice 数、量化噪声、部分和累加误差与拒识率之间的关系。

主线在当前 BCICIV 和 BNCI 平均准确率上未稳定超过 LDA baseline，说明经验库特化仍依赖被试、校准量、top-$K$ 和条目质量。FBCSP 滤波与空间投影在在线系统中也可能构成延迟瓶颈。Cyton 上位机与真实闭环未完成，因此本文结论只能覆盖公开数据回放、软件后端接口和计算账本，不能外推为真实被试部署效果。
...
第二步是建立混合精度评估。对候选单次 4-bit、8-bit 逻辑精度的 4-bit 位权拆分、原生更宽物理单元和可选量化感知训练进行对比；逐算子搜索满足 command accuracy、accepted accuracy 和 reject rate 的最低精度，并同时统计 slice/tile 次数、噪声鲁棒性、延迟和能耗，形成精度--效率 Pareto 选择，而不是统一使用最高精度。
...
当前 Gazelle 模拟器已经接入单窗口完整前向链，并记录了 $1{,}089{,}752$ 个 bit-sliced tiles。模拟器的 476.405 s 主要来自逐 tile、逐 slice 的软件模拟开销，不代表评估板实机速度；按照产品手册标称算力进行同口径估算，单窗口板端计算约为 0.42 s，短于约 3 s 的 EEG 决策窗口，说明 Gazelle 平台具备支撑本项目实时推理的计算时间条件。因而，Gazelle 对本项目的重要性不仅在于替换某个数字矩阵乘，更在于提供一条可量化、可调度、可验证并可进一步实机部署的光计算主干，使“EEG 特征提取--个体化经验库--多候选扫描--拒识/命令”能够形成完整的光电混合系统。
...
表~\ref{tab:ops}按主线的一个决策窗口列出主要线性算子。这里取 $B=6$ 个频带、$C=8$ 个通道、每类 4 条 FBCSP 滤波器、$D=32$ 个特征和 $K=8$ 个候选。表中的示例使用 BCICIV\_1\_asc 的 $T=300$（marker 后 1.0--4.0 s，100 Hz）。BNCI2014\_004 的 trial 长度为 3 s、采样率为 250 Hz，应按 $T=750$ 重新计算。IIR 滤波和 var、log、softmax 等非线性计算不放入矩阵 MAC；赛题最终是否统计这些计算，以官方口径为准。
{\small
...
OVR CSP 投影 & $B\times3\times(4\times8)(8\times T)$ & $6\times3\times4\times8\times300=172{,}800$ & MatrixOps 接管 & bit-sliced 软件模拟 \\ \hline
标准化仿射 & $(32+1)\times32$ & $1{,}056$ & MatrixOps 仿射 & bit-sliced 软件模拟 \\ \hline
MLP 第一层 & $(64\times33)(33\times1)$ & $2{,}112$ & MatrixOps MVM & bit-sliced 软件模拟 \\ \hline
MLP 第二层 & $(32\times65)(65\times1)$ & $2{,}080$ & MatrixOps MVM & bit-sliced 软件模拟 \\ \hline
候选头扫描 & $8\times(3\times33)(33\times1)$ & $792$ & 80 个 $2\times8$ tile & 单次 4-bit 软件模拟 \\ \hline
...
线性层矩阵大小、计算量和光计算标记 & 第 3 章与附录 A；backend 接管、软件执行核和硬件未实测边界明确 \\ \hline
