...
在 BNCI2014\_004 的 9 位被试实验中，主线在 1296 个评估窗口上的平均准确率为 74.92\%，接受窗口准确率为 76.22\%，拒识率为 1.62\%。同协议下，LDA baseline 的平均准确率为 73.46\%，小型 MLP 为 74.38\%。主线总体平均准确率略高于 LDA 和小型 MLP，但与 LDA 仅相差 1.46 个百分点，尚不足以认为三条线路已经拉开稳定差距，也不能据此断言经验库特化能够普遍提升精度。其已验证价值是把少量会话校准、候选模型特化和 photonic scan 连成可复现路径。后续应在经验库完成被试/会话特化后重点报告每位被试的收益差异，并对校准窗口数、top-$K$ 和经验库规模进行消融，以验证各算法之间的真实差距。
