...
正式前向实验通过 Gazelle 光计算模拟器执行。现有模拟器证据包括 backend 标识、量化参数、tile/slice 调度和阶段耗时，不包括真实 ADC/DAC、器件标定、功耗或芯片时延。
...
逐算子的目标是找到满足任务指标与数值稳定性的最低逻辑精度。自适应/固定 8-bit A/B 按相同原始 EEG 窗口、模型参数和拒识协议在光计算模拟器上执行。当前数据和精度策略仍待完善，不作为定版结论；正式结论还需要完整测试集和真实硬件验证。后续建议扩展逻辑位宽、物理位宽与噪声等级实验矩阵。
...
按当前 forward-only 口径，全部前向线性 MAC 已路由到 photonic backend，候选头是重点单次 4-bit、$2\times8$ tile 路径；现有证据覆盖公开数据回放、Gazelle 模拟器接口、量化调度和计算账本，真实评估板上的端到端时延、数值误差与功耗仍需进一步实测。主线已经验证“会话校准--经验库特化--候选扫描--拒识”的完整机制，当前平均准确率略高于 LDA baseline，但尚未拉开稳定差距；仍需在经验库完成被试/会话特化后，通过消融、逐被试分析和真实被试实验验证各算法差距。Cyton 上位机与真实在线闭环也尚未完成。后续工作的核心是保持现有统一接口不变，将模拟器路径迁移到 Gazelle 实机并完成真实 EEG 闭环验证。
